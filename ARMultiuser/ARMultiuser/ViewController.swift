...
//        }
        ...
        // Set the position of the text node to the hit test location
...
//        textNode.eulerAngles.y = camera?.eulerAngles.y ?? 0
    ...
    @IBAction func handleSceneTap(_ sender: UITapGestureRecognizer) {
        ...
        // Prompt the user to enter annotation text
        let alertController = UIAlertController(title: "New Annotation", message: "Enter text for the new annotation", preferredStyle: .alert)
        ...
        let confirmAction = UIAlertAction(title: "OK", style: .default) { [weak self, weak alertController] _ in
            ...
            // Create a new annotation with the input text
            let text = alertController.textFields?[0].text ?? ""
            
            
            // Place an anchor for a virtual character. The model appears in renderer(_:didAdd:for:).
            let anchor = ARAnchor(name: text, transform: hitTestResult.worldTransform)
            sceneView.session.add(anchor: anchor)
            
//            // Add the text node to the scene
//            self.sceneView.scene.rootNode.addChildNode(textNode)
//
//            // Store the annotation node for later reference
//            self.annotationNode = textNode
